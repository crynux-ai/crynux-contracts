...
describe("Task", async () => {
    it("push task to queue when there is no available nodes", async() => {
        ...
        let tx = contract.createTask(
            taskType, taskHash, dataHash, 8, taskFee, cap);
...
describe("Task", async () => {
    it("select correct task (sd)", async() => {
        ...
        let contract = await v.taskInstance.connect(v.user);
        let tx = contract.createTask(0, taskHash, dataHash, 8, taskFee, cap);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        let logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
        ...
        tx = contract.createTask(1, taskHash, dataHash, 8, taskFee, cap);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        ...
        tx = contract.createTask(0, taskHash, dataHash, 16, taskFee, cap);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
...
describe("Task", async () => {
    it("select correct task (gpt)", async() => {
        ...
        let contract = await v.taskInstance.connect(v.user);
        tx = contract.createTask(0, taskHash, dataHash, 8, taskFee, cap);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        let logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
...
describe("Task", async () => {
    it("select task with higher task fee", async() => {
        ...
        let contract = await v.taskInstance.connect(v.user);
        let tx = contract.createTask(
            0, taskHash, dataHash, 8, ethers.parseUnits("30", "ether"), 1);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        let logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
        ...
        tx = await contract.createTask(
            0, taskHash, dataHash, 8, ethers.parseUnits("60", "ether"), 2);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
        ...
        tx = await contract.connect(v.user).createTask(
            0, taskHash, dataHash, 8, ethers.parseUnits("35", "ether"), 1);
        await expect(tx).emit(contract, "TaskPending").withArgs(
            anyValue, anyValue, v.user.address, anyValue, anyValue);
        tx = await (await tx).wait()
        logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
...
describe("Task", async () => {
    it("select task correctly when a node is slashed", async () => {
        ...
        let tx = await v.taskInstance.connect(v.user).createTask(
            0, taskHash, dataHash, 8, ethers.parseUnits("40", "ether"), 1);
        tx = await (await tx).wait()
        let logs = tx.logs.filter((x) => x.constructor.name == "EventLog");
        let taskId = logs[0].args.taskId;
        let nodeRounds = {};
    ...
    it("should abort the cheapest task when task queue is full", async () => {
        ...
        for (let i = 0; i < 3; i++) {
            const tx = v.taskInstance.connect(v.user).createTask(
                0, taskHash, dataHash, 8 + i, ethers.parseUnits(`${10 * (i + 1)}`, "ether"), 1);
